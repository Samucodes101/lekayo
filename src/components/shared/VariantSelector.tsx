...
export default function VariantSelector({ variants, selectedColorId, selectedSize, onColorChange, onSizeChange }: VariantSelectorProps) {
  const colors = Array.from(new Map(variants.filter(v => v.colorId).map(v => [v.colorId, v.color])).values()).filter(Boolean) as Color[]
          ...
          <div className="flex gap-2">
            {colors.map((color) => (
              <button
                ...
                className={`w-8 h-8 rounded-full border-2 ${selectedColorId === color.id ? "border-black" : "border-transparent"}`}
